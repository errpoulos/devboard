...
import { useCompanies, useCompanyNotes, useCreateCompanyNote } from '@/features/crm/hooks/useCrm'
  ...
  if (!company) {
    return (
      <div className="p-6 max-w-3xl">
        ...
        <p className="text-[13px] text-[#8a8f98]">Company not found.</p>
      </div>
    )
  }
    ...
    <div className="p-6 max-w-3xl space-y-6">
      ...
      <div className="rounded-md p-5 border border-[#23252a]" style={{ background: '#0f1011' }}>
        <div className="flex items-center gap-3 mb-4">
          <div className="w-9 h-9 rounded-full bg-[#23252a] flex items-center justify-center shrink-0">
            <Building2 size={16} className="text-[#8a8f98]" />
          </div>
          <div>
            ...
            {company.domain && (
              <p className="text-[12px] text-[#8a8f98]">{company.domain}</p>
            )}
          </div>
        </div>
        ...
        {company.industry && (
          <dl className="grid grid-cols-2 gap-x-6 gap-y-2.5">
            <dt className="text-[11px] text-[#6b7280] uppercase tracking-[0.04em]">Industry</dt>
            <dd className="text-[12px] text-[#c4c7cc]">{company.industry}</dd>
          </dl>
        )}
      </div>
      ...
      <div className="rounded-md p-5 border border-[#23252a]" style={{ background: '#0f1011' }}>
        ...
        {notesLoading ? (
          ...
          <NotesPanel
            notes={notes}
            invalidateKey={queryKeys.companyNotes(companyId)}
